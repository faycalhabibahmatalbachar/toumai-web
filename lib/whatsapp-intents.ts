...
/**
 * Le classifieur ci-dessous ne doit reconnaître QUE le cycle de vie du
 * connecteur (connexion, QR, état, numéro du compte lié…). Les vraies actions
 * WhatsApp doivent continuer vers le modèle et ses tools : contacts, messages,
 * groupes, statuts, profil, recherche, synchronisation, etc.
 *
 * C'est volontairement plus strict qu'un classifieur de sujet. Le fallback
 * historique « le message contient WhatsApp => status » transformait toutes
 * les commandes métier en carte de connectivité.
 */
const BUSINESS_ACTION = /(?:
  conversations?|chats?|messages?|contacts?|groupes?|groups?|channels?|cha[iî]nes?|
  synchronis|sync|chercher|rechercher|trouver|liste|lister|afficher|montre|montrer|
  cr[eé]er|ajouter|retirer|supprimer|modifier|changer|mettre|publier|poster|status\b|statut\b|
  envoyer|r[eé]pondre|r[eé]agir|transf[eé]rer|forward|archiver|d[eé]sarchiver|bloquer|d[eé]bloquer|
  photo|image|vid[eé]o|document|fichier|appel|call|lire|lecture|r[eé]sumer|analyse|analyser
)/ix;
...
const NUMBER = /(?:
  (?:quel(?:\s+est)?|c['’]?est\s+quoi|donne(?:-moi)?|affiche)\s+(?:mon\s+)?num[eé]ro|
  mon\s+num[eé]ro(?:\s+whats?app)?\s*(?:c['’]?est\s+quoi)?\s*\??$|
  my\s+(?:whats?app\s+)?number\s*\??$|
  num[eé]ro\s+(?:du\s+)?compte\s+(?:li[eé]|whats?app)
)/ix;
...
const STATUS = /(?:
  (?:statut|[ée]tat|status)\s+(?:de\s+)?(?:la\s+)?(?:connexion|liaison|connecteur)|
  (?:connexion|liaison|connecteur)\s+(?:whats?app\s+)?(?:est[- ]?elle\s+)?(?:active|ouverte|connect[eé]e?)|
  (?:mon\s+)?whats?app\s+(?:est[- ]?il\s+|est\s+)?connect[eé]\s*\??|
  (?:is\s+)?(?:my\s+)?whats?app\s+connected\s*\??|
  v[eé]rifie\s+(?:si\s+)?(?:mon\s+)?whats?app\s+(?:est\s+)?connect[eé]
)/ix;

function isBusinessAction(value: string): boolean {
  // Les verbes du cycle de vie du connecteur gardent la priorité. « Déconnecte
  // WhatsApp » est une action, mais c'est précisément une action de connexion.
