...
function Receipt({ reference }: { reference: string }) {
  ...
  if (payment.statut !== "success") return <section className="billing-card billing-state"><StatusBadge status={payment.statut}/><h1>Le reçu n’est pas encore disponible.</h1><p>Un reçu est émis uniquement pour un paiement confirmé.</p><Link className="billing-button" href={`/abonnement/retour?ref=${encodeURIComponent(reference)}`}>Suivre le paiement</Link></section>;
