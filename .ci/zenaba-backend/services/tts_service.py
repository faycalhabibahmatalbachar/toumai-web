...
from services import prononciation
...
async def stream_zenaba_live(
    ...
    """Flux WAV natif de Zenaba/Pocket TTS, français uniquement.

    Le flux est relayé chunk par chunk depuis le service CPU Pocket. Une
    fermeture client (Stop, barge-in, navigation) ferme la connexion amont.
    Aucun autre moteur n'est essayé en Voice V1.
    """
    settings = get_settings()
    if not settings.toumai_voice_v1_enabled:
        raise TtsProviderError(
            "pocket",
            "Le streaming live Zenaba nécessite Toumaï Voice V1.",
            409,
        )
    if not _is_french_v1(language):
        raise TtsProviderError(
            "pocket",
            "Toumaï Voice V1 lit uniquement le français.",
            422,
        )
    ...
    if not _looks_french_v1(text):
        raise TtsProviderError(
            "pocket",
            "Zenaba lit uniquement le français pour le moment.",
            422,
        )

    spoken = prononciation.pour_la_voix(text)
    async for chunk in stream_pocket_audio(spoken):
        yield chunk
...
def _tts_provider_order(prefer_kokoro: bool = False, prefer_voxcpm: bool = False) -> List[str]:
    ...
    En Toumaï Voice V1, Zenaba/Pocket TTS est l'unique moteur actif. Les anciens
...
async def synthesize(
    ...
    # V1 : une seule identité vocale, français uniquement, fail-closed.
    if settings.toumai_voice_v1_enabled:
        if not _is_french_v1(language):
            raise TtsProviderError(
                "pocket",
        ...
        spoken = prononciation.pour_la_voix(text)
        audio = await synthesize_pocket(spoken)
...
async def synthesize_stream(
    ...
    *,
    ...
    En Toumaï Voice V1, le flux est français/Zenaba Pocket uniquement et fail-closed :
    ...
    # Avant le DÉCOUPAGE, et pas après : la prononciation doit rester stable
    ...
    if settings.toumai_voice_v1_enabled:
        ...
        # Pocket TTS n'est pas thread-safe et le service sérialise les requêtes.
        # Un lookahead >1 créerait seulement des requêtes en attente susceptibles
        # d'expirer derrière le verrou ; en V1 on garde une génération à la fois.
        lookahead = 1
    else:
