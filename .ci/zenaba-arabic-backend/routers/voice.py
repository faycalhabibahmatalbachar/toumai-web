...
async def _sonde_zenaba_reelle() -> dict:
    """Une vraie synthèse, une seule phrase, pour savoir si la voix sort.

    POURQUOI ELLE N'EST PAS SUR /health
    ------------------------------------
    Le contrat Gradio du Space peut répondre pendant que la génération échoue :
    c'est ce qui s'est produit le 20/09/2026, tous les booléens au vert et la
    synthèse en erreur. Seule une synthèse réelle départage. Mais elle coûte un
    passage ZeroGPU, et sur `/health`, qui est public, n'importe qui pouvait
    vider le quota du Space en boucle. Elle vit donc ici, derrière
    l'authentification, et dans `scripts/smoke_zenaba.py` hors ligne.

    La réponse ne porte ni le son ni le texte d'une erreur amont : la taille,
    l'en-tête, la durée, et le code ou le NOM de l'échec.
    """
    import time

    debut = time.monotonic()
    try:
        audio = await tts_service.synthesize_chatterbox("Bonjour, ici Zenaba.")
        return {
            "ok": True,
            "bytes": len(audio),
            "riff": audio[:4] == b"RIFF" and audio[8:12] == b"WAVE",
...
async def voice_health(
    ...
    `?synthese=1` ajoute une synthèse Zenaba réelle. Elle coûte un passage
    ZeroGPU, d'où le drapeau explicite : un diagnostic ne doit pas consommer
    le quota du Space à chaque contrôle de santé.
    """
    ...
    providers = {
        ...
        "toumai_voice_arabic_enabled": bool(
            getattr(settings, "toumai_voice_arabic_enabled", False)
        ),
        "tts_chatterbox_configured": bool(settings.chatterbox_tts_url),
    ...
    zenaba: dict = {}
    try:
        zenaba = await tts_service.chatterbox_health_check()
    except Exception as e:
        zenaba = {
            "ok": False,
            "configured": bool(settings.chatterbox_tts_url),
            "voice": settings.toumai_voice_name or "Zenaba",
            "engine": tts_service.CHATTERBOX_ENGINE_ID,
            "reference": tts_service.CHATTERBOX_REFERENCE_ID,
            "error": str(e),
        }
    ...
    sonde = await _sonde_zenaba_reelle() if synthese else None
...
async def synthesize(
    ...
    En Voice V1, Zenaba/Chatterbox Multilingual V3 est l'unique moteur.
    Le français est actif ; l'arabe est derrière TOUMAI_VOICE_ARABIC_ENABLED.
...
async def synthesize_live(
    ...
    """Zenaba en WAV brut pour français ou arabe autorisé.

    CE QUE CETTE ROUTE FAIT, ET CE QU'ELLE NE FAIT PAS
    ---------------------------------------------------
    Elle évite le détour base64 du flux NDJSON : le navigateur reçoit un
    `audio/wav` qu'il peut décoder sans rien reconstruire. Mais Chatterbox V3
    rend un WAV COMPLET par segment, pas un flux PCM continu : l'audio ne
    commence donc pas avant la fin de la génération du segment. Appeler cela
    du « vrai streaming » ferait chercher une latence qui n'existe pas à cet
    endroit. Le gain de latence se joue dans le découpage en phrases, côté
    `/voice/synthesize/stream`.

    L'en-tête RIFF/WAVE est validé avant d'ouvrir la réponse, pour qu'une
    panne amont devienne une erreur HTTP et non un lecteur muet.
    """
    ...
    async def _wav():
        try:
            ...
            # navigateur ferme aussi la génération amont.
            await stream.aclose()
