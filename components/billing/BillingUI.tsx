...
export function BillingShell({ children }: { children: ReactNode }) {
  return <div className="billing-shell"><header className="billing-header"><Link href="/" className="billing-brand"><Logo size={36} /><span>Toumaï <span className="billing-muted">AI</span></span></Link><Link href="/contact">Besoin d’aide ?</Link></header><main className="billing-main">{children}</main><footer className="billing-footer"><span>Toumaï AI · À vos côtés, à chaque étape.</span><nav aria-label="Informations de facturation"><Link href="/privacy">Confidentialité</Link><Link href="/terms">Conditions</Link><Link href="/security">Sécurité</Link><Link href="/contact">Contact</Link></nav></footer></div>;
}
