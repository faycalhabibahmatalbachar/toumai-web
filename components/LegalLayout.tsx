...
export function LegalLayout({
  ...
  const content = Children.map(children, (child) => {
    if (!toc || !isValidElement<{ children?: ReactNode; id?: string; className?: string }>(child) || child.type !== "h2") return child;
    const id = child.props.id || `section-${sections.length + 1}`;
    sections.push({ id, label: child.props.children });
    return cloneElement(child, { id, className: "scroll-mt-6" });
  });
  return (
    <div className="flex min-h-dvh flex-col">
      {path && <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "WebPage", url: `https://toumaiai.com${path}`, name: title, inLanguage: "fr", isPartOf: { "@id": "https://toumaiai.com/#website" } }).replace(/</g, "\\u003c") }} />}
      ...
      <main className="mx-auto w-full max-w-3xl min-w-0 flex-1 px-5 pb-20 pt-8 [overflow-wrap:anywhere] sm:px-8">
        ...
        {toc && <nav aria-label="Sommaire" className="mt-8 rounded-2xl border border-[var(--border)] bg-[var(--bg-secondary)] p-5 text-sm">
          <p className="mb-3 font-semibold">Dans ce document</p>
          <ol className="grid gap-3 sm:grid-cols-2">{sections.map(({ id, label }) => <li key={id}><a className="underline decoration-[var(--border)] underline-offset-4 hover:decoration-current" href={`#${id}`}>{label}</a></li>)}</ol>
        </nav>}
